...
def compute_gather(
    ...
    for row in tqdm(table_mapping.itertuples(), total=len(table_mapping), desc="Computing md5sum.."):
        ...
        base_path = os.path.join(exp_dir, str(row.sample_id)) if has_sample_id and nested else exp_dir
        if os.path.exists(base_path):
        
            r1_path = os.path.join(base_path, row.forward)
            r2_path = os.path.join(base_path, row.reverse)
    ...
    if md5_df is not None:
        updated_table = pd.merge(table_mapping,md5_df ,how='left', on='sample_alias')

    metadata_dir = os.path.dirname(metadata_path)
    updated_mapping_file = os.path.join(metadata_dir,'mapping_table.tsv')
...
def create_run(
    ...
    run_xml = []
    required_cols = ["forward", "reverse","sample_alias","r1_md5sum","r2_md5sum"]
